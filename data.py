...
def dev_preprocess(x, y):
    ...
    # 读取并缩放图片
    x = tf.io.read_file(x)
    x = tf.image.decode_jpeg(x, channels=3)# 图片解码 解码图像的颜色通道数量为3
...
train_samples = int(total_samples * settings.TRAIN_SCALE)  # 训练集样例数
dev_samples = int(total_samples * settings.VAL_SCALE)  # 验证集样例数
